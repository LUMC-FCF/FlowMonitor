...
```{r (Post) Adding .id column to pass/fail requirement dataframe , warning=TRUE}
DF.w_id_columnsp_f <- setDT(DF.id_columnsp_f, keep.rownames = ".id")
```


#Renaming undefined column "Pass_and_Fail"
```{r (Post) Renaming undefined column "Pass_and_Fail", warning=TRUE}
DF.w_id_columnsp_f2 <-  DF.w_id_columnsp_f %>% rename(Pass_and_Fail = V1)
```
...
TableFinal_Aurora <- merge.data.frame(TableFinal_Aurora ,DF.w_id_columnsp_f2, by.x= ".id", by.y = ".id")
```
