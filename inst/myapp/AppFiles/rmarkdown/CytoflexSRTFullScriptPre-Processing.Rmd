...
#File.List.CytoflexSRT_All<-list.files(path = "//vf-flowcytometrie-data/flowcytometrie-data$/fcf/Instrument performance tracking files/Beckman Coulter", pattern = ".csv", recursive=TRUE, full.names = TRUE)
...
Tables.list<- lapply(File.List.CytoflexSRT_All, fread, header= TRUE, skip = "Gain", fill = TRUE, na.strings= "")
```
...
Tablespt3.list <- lapply(File.List.CytoflexSRT_All, fread, header= TRUE, 
                         skip = "Item", fill = TRUE, sep = ",")
```
...
```{r removing target value column}

DF.id.cytoflex3 <- DF.id.cytoflex3[,!names(DF.id.cytoflex3) %in% "Target"]
```
...
```{r reorder table alphabetically}
...
TableFinal_CytoflexSRT <- TableFinal_CytoflexSRT %>% arrange(Instrument, Laser_Detector,Date)
```
